...
(function () {
  ...
  window.BioLB = {
    ...
    async hasSubmitted(key){
      if (!enabled || !this.user) return null;
      await waitReady();
      return window._lbImpl.checkSubmitted(key, this.user.uid);
    },
  ...
  script.textContent = `
    ...
    window._lbImpl = {
      ...
      checkSubmitted: async (key, uid) => {
        const col = collection(db, 'scores');
        const q = query(col, where('uid','==',uid), where('chapter','==',key), limit(1));
...
(function(){
  ...
  function render(user){
    ...
    if (user) {
      el.innerHTML =
        `<a class="lb-link" href="${linkToLeaderboard()}" title="Leaderboard">🏆${isNarrow ? '' : '<span class="lb-link-text"> Leaderboard</span>'}</a>` +
        `<img class="lb-avatar" src="${user.photo||''}" alt=""/>` +
        `<span class="lb-name">${user.name||'Player'}</span>` +
        `<button class="lb-btn lb-out">${isNarrow ? 'Out' : 'Sign out'}</button>`;
      el.querySelector('.lb-out').onclick = ()=>window.BioLB.signOut();
    } else {
      el.innerHTML =
        `<a class="lb-link" href="${linkToLeaderboard()}" title="Leaderboard">🏆${isNarrow ? '' : '<span class="lb-link-text"> Leaderboard</span>'}</a>` +
        `<button class="lb-btn lb-in">${isNarrow ? 'Sign in' : 'Sign in with Google'}</button>`;
