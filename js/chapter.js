...
async function submitGameATP(key, atp, max, hostEl) {
  ...
  // Reward animation — molecules fly to the score display
  if (atp > 0) {
    const rewardTarget =
      (hostEl && (hostEl.querySelector('.atp-total') || hostEl.querySelector('strong'))) || hostEl;
    if (rewardTarget) {
      // Tiny delay so the result panel is laid out before we measure it
      setTimeout(() => spawnATPMolecules(rewardTarget, atp), 80);
    }
  }
  ...
  if (!window.BioLB.user) {
    toast.innerHTML = `<strong>You earned ${atp} ${ICON}.</strong> Sign in to save it to the leaderboard. ` +
      `<button class="btn" style="margin-left:8px">Sign in</button>`;
    toast.querySelector('button').onclick = async () => {
      try {
        await window.BioLB.signIn();
        await window.BioLB.submitATP({ key, atp, max });
        toast.innerHTML = `Saved — ${atp} ${ICON} on ${key}.`;
        showRankingReveal();
      } catch (e) { toast.className = 'lb-toast bad'; toast.textContent = 'Sign-in failed.'; }
    };
    return;
  }
  try {
    await window.BioLB.submitATP({ key, atp, max });
    toast.innerHTML = `Saved — ${atp} ${ICON} earned.`;
    showRankingReveal();
  } catch (e) {
